...
{
//左腿
modelViewMatrix = mult(modelViewMatrixbase, translate(0.9, -0.5, 0.0)); 
modelViewMatrix = mult(modelViewMatrix, rotate(-170, 0, 0, 1 ));
lowerArm();
...
function headd()
{
    modelViewMatrix = mult(modelViewMatrixbase, translate(0.0, 8.0, 0.0)); 
    modelViewMatrix = mult(modelViewMatrix, rotate(0, 0, 0, 1 ));
    head();
}
function based()
{
    ...
    modelViewMatrixbase = modelViewMatrix;
    base();
}
...
{
//右臂
modelViewMatrix = mult(modelViewMatrixbase, translate(-2.0, 6.5, 0.0)); 
modelViewMatrix = mult(modelViewMatrix, rotate(140, 0, 0, 1 ));
lowerArm();

modelViewMatrix  = mult(modelViewMatrix, translate(0.0, 4.0, 0.0));
modelViewMatrix  = mult(modelViewMatrix, rotate(thetaWave, 0, 0, 1) );
upperArm();
}
...
function waver(a){
    if(a>-90)
    setTimeout(function(){a-=30; thetaWave = a; waver(a)},500);
    else{
        wavel(-90);
    }
}
...
function wavel(a){
    if(a<0)
    setTimeout(function(){a+=30; thetaWave = a; wavel(a)},500);
}
