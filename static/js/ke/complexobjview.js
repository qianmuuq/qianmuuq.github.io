...
var clearColor = vec4.fromValues( 0.7216, 0.1373, 0.1451, 1.0 );
...
function leftc(){
    changemethod(600,300);
}

function changemethod(a,b){
    if(flag==1){
        if(b<150){
            changea = 6;
            dy = -11;
            document.getElementById('textt').innerHTML = "头部"
        }else if(b>520){
            changea = 6;
            dy = -0.5;
            document.getElementById('textt').innerHTML = "腿部"
        }else if(a>520){
            changea = 5;
            dx = -4.2;
            dy = -5.3;
            document.getElementById('textt').innerHTML = "右手"
        }else if(a<290){
            changea = 5;
            dx = 4.1;
            dy = -5.3;
            document.getElementById('textt').innerHTML = "左手"
        }
        buildModelViewProj();
        flag = 0;
    }else if(flag==0){
...
function handleMouseDown(event) {
    ...
    changemethod(lastMouseX,lastMouseY);
}
...
window.onload = function initWindow(){      //初始化
    ...
    gl.clearColor(0.7216, 0.1373, 0.1451, 0.8);
...
function buildModelViewProj(){
    ...
    //mat4.translate(mvMatrix, mvMatrix, vec3.fromValues(dx, dy, dz));
    mat4.scale(mvMatrix, mvMatrix, vec3.fromValues(sx*changea, sy*changea, sz*changea));
